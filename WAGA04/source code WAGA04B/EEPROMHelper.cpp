...
void EEPROM_setup()
{
  ...
  //settings->connection=CON_SERIAL;  // WAGA use this line if need reset CON-SERIAL connection 230425
 
  if(settings->connection==CON_BLUETOOTH_SPP){
    settings->connection=CON_BLUETOOTH_LE;  //WAGA 260425  SPP is causing resets
  }
...
void EEPROM_defaults()
{
  ...
  eeprom_block.field.settings.portland        = SCREEN_PORTRAIT;  //  WAGA01
  eeprom_block.field.settings.vmode           = VIEW_MODE_RADAR;
  eeprom_block.field.settings.zoom            = ZOOM_LOW;
  eeprom_block.field.settings.waypoint        = YBEV;  //WAGA02
  eeprom_block.field.settings.OTI_Danger      = 0; // WAGA04
  eeprom_block.field.settings.OTI_HighLow     = 0; // WAGA04
