...
void Traffic_Add()
{
    ...
    */
      ...
      // Lookup the Comp_ID - KW 080426
          ...
          //Serial.print("DB_query found = ");Serial.println(found);
          if (found==1){
...
void Traffic_loop()
{
 ...
 if (settings->voice==VOICE_BUZZER && GliderOnTow()==0){
    Buzzer();  // only run Buzzer function if there is no glider on tow.  GliderOnTow() returns false if Alarm>0
 }
...
// FUNCTION: Sort in threat/dist order  WAGA 260325
int Threat_Assess()  // assess by alarm level then distance.  Returns J count of traffic.  Don't include static PFLAA targets in traffic
{
  //Serial.println("==> Threat Assess called");
  int i,j;
  i=j=0;
     
  for (int i=0; i < MAX_TRACKING_OBJECTS; i++)
  {
    // don't include static traffic unless within 120m (ie at launch).  Ground speed only available with type=1 PFLAA
    if (Container[i].ID && (now() - Container[i].timestamp) <= EPD_EXPIRATION_TIME && 
        !(Container[i].packet_type==1 && Container[i].GroundSpeed==0 && Container[i].distance > 150))
    {
  ...
  // PUT THE OTI CODE HERE 160426.  call a function, then active the buzzer
 //Serial.println("==> here will be call OTI function");
 return j;
}
