...
const String Nav_Names[6]= {"YBEV","YCUN","YNRG","YKEP","YNRM","YCNK"}; // Jacob special 060826
const double Nav_Waypoints[6][2] ={ 
  {-32.125,116.950},  // YBEV
  {-31.622,117.217},  // YCUN
  {-32.930,117.079},  // YNRG
  {-30.891,150.526}, // YKEP
  {-32.216,148.224}, // YNRM
  {-32.549,151.024} // YCNK (no comma on final line)
...
#define ICON_RMI_POINTS 5  // KW 211025
const float RMI_Points[ICON_RMI_POINTS][2] = {{0,-65},{-6,-58},{6,-58},{0,-58},{0,65}};
#define HGTBAR_POINTS 5  // KW 081125
const float HGTBAR_Coords[HGTBAR_POINTS][2] = {{-30,-4},{-10,-24},{-10,-12},{-10,4},{-10,16}}; // first pair is centre line, then 4x boxes from top to bottom
#define HGTBAR_DIV 14
...
static void EPD_Draw_NavBoxes() 
{
 // Serial.println("TIMER E4  - start draw_navboxes");// WAGA debug timings 140425
  ...
  static char *u_dist, *u_dist_m, *u_alt,*u_spd;
...
// --- top nav boxes
  ...
  // NavBox1 ========= Traffic count and alarm_level is common to Radar and Nav views.  
    switch(EPD_view_mode)
    {
      case VIEW_MODE_RADAR:
      {
        ...
        display->setCursor(navbox1.x + 55, navbox1.y + 10);
        ...
        // Threat_Assess() returns count of traffic
        if (Threat_Assess()>0){
          ...
          if(traffic[0].fop->alarm_level>0){
            ...
            //-- display Traffic number--WAGA02---
            display->setFont(&FreeSerifBold12pt7b);
            display->setCursor(navbox1.x + 30, navbox1.y + 30);
            navbox1.value= Traffic_Count()-FT;
      ...
      case VIEW_MODE_NAV:
      {
       ...
       display->setFont(&FreeMonoBold9pt7b);
       display->setCursor(navbox1.x + 20, navbox1.y + 32);
       display->print(Nav_Names[settings->waypoint]);;

        display->setCursor(navbox1.x + 65, navbox1.y + 10); 
        display->setFont(&Picopixel);
        display->print ("RMI");

      }
    }
// here print Navbox1.title title
    display->setFont(&Picopixel);
    ...
    switch(EPD_view_mode){
      ...
      //-----------------display nearest distance-------- 
       memcpy(navbox2.title, NAVBOX2_TITLE, strlen(NAVBOX2_TITLE)); // kw 2107
       if (Threat_Assess()>FT) {

          display->setCursor(navbox2.x + 55, navbox2.y + 5 + tbh);
          display->setFont(&Picopixel);
          ...
          display->setFont(&FreeSerifBold12pt7b);
          display->setCursor(navbox2.x + 10, navbox2.y + 32);
 
          if (traffic[FT].fop->distance * disp_dist <1000 && settings->units !=UNITS_IMPERIAL){
            navbox2.value = traffic[FT].fop->distance *disp_dist;
            snprintf(info_line, sizeof(info_line), "%3.0f",(float)navbox2.value);
            u_dist = u_dist_m;
          }else{
           navbox2.value = traffic[FT].fop->distance * disp_dist;
           snprintf(info_line, sizeof(info_line), "%4.1f",(float)navbox2.value/1000);
          }

          
          display->print(info_line);

          display->setCursor(navbox2.x + 60, navbox2.y + 32); 
          display->setFont(&FreeMonoBold9pt7b);
  ...
  //====== BOTTOM NAV BOXES ===========
  {
    ...
    switch(EPD_view_mode){ 
      case VIEW_MODE_RADAR:
      {
        ...
        if (Threat_Assess()==FT){
          break;  // WAGA 070425 if there is no traffic other than on tow, then nothing to set navboxes 3 and 4 values
        }
   
        navbox3.value = int(traffic[FT].fop->RelativeVertical *disp_alt); // vertical distance
        ...
        display->setFont(&Picopixel);
        uint32_t id =0;
        if (traffic[FT].fop->ID){
          id = traffic[FT].fop->ID; 
        ...
        if (traffic[FT].fop->ID){
          display->print(traffic[FT].fop->Comp_ID);
           //Serial.print("XXX traffic 0 Comp_ID  -> "); Serial.println(traffic[0].fop->Comp_ID);  // KW 090426
        }
        /*
    ...
    // here print navbox3 and navbox4 titles
    display->setFont(&Picopixel);
  ...
  //Serial.println("=== text boxes end");  // debug 160926
}
...
{ //--A
  Serial.println("==> start Draw_radar"); // WAGA debug 160926
  int16_t  tbx, tby;
  uint16_t tbw, tbh;
  int16_t x;
  int16_t y;
  ...
  uint8_t db=DB_OGN; // KW 181025
  char id_text [TEXT_VIEW_LINE_LENGTH]; // KW 181025
  char id2_text [TEXT_VIEW_LINE_LENGTH]; // 181025
  String tail;
  ...
  static int16_t radar_center_x = radar_w / 2;
  static int16_t radar_center_y = radar_y + radar_w / 2;
  int16_t radius = radar_w / 2 - 2;

  static int16_t ontow_x = radar_x + radar_w /2;  // KW 180726
  static int16_t ontow_y_status = radar_y + radar_w;
  static int16_t ontow_y_ID = radar_y + radar_w;
  //static int16_t ontow_w = radar_w /2;
  //static int16_t ontow_h = radar_w /4;
...
// WAGA 231025 -- when alarms displayed, surpress target display
           
  if (Threat_Assess()!=0 && traffic[0].fop->alarm_level>0 && traffic[0].fop->Source!=6)
  { 
    FT=0; // first alarmed threat is always taffic[0]
    //------FLARM ALARM LOOK-OUT POINTER -----

    if(EPD_view_mode==VIEW_MODE_NAV){
      EPD_view_mode = VIEW_MODE_RADAR;
    }
    //display->setFont(&FreeMonoBold9pt7b);
   // display->getTextBounds("LOOKOUT", 0, 0, &tbx, &tby, &tbw, &tbh);
    //display->setCursor(radar_center_x - tbw/2,radar_center_y +tbh/2);
   // display->print("LOOKOUT");
    ...
    {
      // was drawTriangle in WAGA01
      display->fillTriangle(radar_center_x + (int16_t) round(epd_Points[3][0]),
    ...
    {
      display->fillTriangle(radar_center_x + (int16_t) round(epd_Points[6][0]),
    ...
    // --DRAW HEIGHTBAR centreline and boxes  ---- KW 030426

      float Vangle=atan(traffic[0].fop->RelativeVertical/traffic[0].fop->distance)/D2R;
      int VangleI = round(Vangle);
      display->setFont(&Picopixel);
      display->setCursor(radar_center_x+16,radar_center_y-12);
      display->print(HGTBAR_DIV);
      display->setCursor(radar_center_x+12,radar_center_y+15);
      display->print(-HGTBAR_DIV);
      //display->setCursor(radar_center_x-25,radar_center_y+7);  // screen prints calculated angle
      //display->print(VangleI);

      if (VangleI >= HGTBAR_DIV)
        {
          HgtBarShow = 1;
         
        } else if (VangleI >= 0 && VangleI < HGTBAR_DIV)
        {
          HgtBarShow = 2;
        ...
        } else
        {
          HgtBarShow = 4;
            ...
            display->drawLine(radar_center_x - (int16_t) HGTBAR_Coords[0][0],
                                radar_center_y,
                                radar_center_x  + (int16_t) HGTBAR_Coords[0][0],
                                radar_center_y,
                                GxEPD_BLACK);
    
    for (int i=1; i < HGTBAR_POINTS; i++)
    {
    
     if (i==HgtBarShow)
      {
        display->fillRect(radar_center_x + (int16_t) HGTBAR_Coords[i][0],
                            radar_center_y + (int16_t) HGTBAR_Coords[i][1],
                           20,
                            8,
                            GxEPD_BLACK);
      ...
      {
        display->drawRect(radar_center_x + (int16_t) HGTBAR_Coords[i][0],
                            radar_center_y + (int16_t) HGTBAR_Coords[i][1],
                           20,
                            8,
                            GxEPD_BLACK);
  ...
  { 
  // ===  NO ALARMS, SO START LOOP READING CONTAINER ===  (not sorted for threat) to display all targets
 
    float trSin = sin(D2R * (float)ThisAircraft.Track);
    float trCos = cos(D2R * (float)ThisAircraft.Track);
    Serial.println("==> start container loop"); // WAGA debug 160926

    for (int i=0; i < MAX_TRACKING_OBJECTS; i++) {
      //Serial.print("=== counting through traffic, i = ");Serial.println(i);  // WAGA debug 160926
        //======= don't show static traffic unless in Nav Mode.  Ground spped only with type=1 PFLAA         
      if (Container[i].ID && (now() - Container[i].timestamp) <= EPD_EXPIRATION_TIME &&
         !(Container[i].packet_type == 1 && Container[i].GroundSpeed == 0 && EPD_view_mode != VIEW_MODE_NAV)) {

        float rel_x;
        float rel_y;
        float tgtSin;
        float tgtCos;
        float rmiSin; // KW 221025
        float rmiCos;
...
//=====================================================================
      ...
      // ====== DRAWING ICONS ON RADAR (dont show if Source=6 is mode-s non- directional, or static PFLAA) ==============
      if(Container[i].Source!=6)
      {
        // Serial.print("--draw icons - i="); Serial.print(i);

               // -- draw a line to threat aircraft    
        //Serial.println("=== about to test for draw line to nearest"); // debug 160926
        if (Container[i].ID && traffic[FT].fop != nullptr){
          if(Container[i].ID==traffic[FT].fop->ID){   
            //Serial.println("==> attempt to draw line");
            display->drawLine(radar_center_x, radar_center_y, radar_center_x + x, radar_center_y - y,GxEPD_BLACK);  
            //Serial.println("==> have drawn line to nearest");  //debug 160926
          }
        }
        else{
           //Serial.println("==> failed test Container[i] && traffic(FT)  i= ");Serial.println(i);  // debug 160926
        }
        ...
        // draw target as a simple triangle
          ...
          // print CompID label beside target ================================================
          int16_t x4 = radar_center_x + x + 6; // -16 = centred
          int16_t y4 = radar_center_y - y + 10;//  -10 = above
          int16_t y5 = radar_center_y - y - 5;//  Tug  +10 = above

          
          
          display->setFont(&FreeMono9pt7b); // KW 040426
          if (Container[i].AcftType==2){   // test for Tug, so as to print Tug ID above glider on tow
            display->setCursor(x4,y5);
            display->print(Container[i].Comp_ID);
            //Serial.print("i = ");Serial.print (i); Serial.print(" TUG Container Comp_ID = ");Serial.println(Container[i].Comp_ID);  // KW 080426
          } else{
            if(Container[i].packet_type == 1 && Container[i].GroundSpeed == 0 && Container[i].AcftType==1){ 
              //TBC - // KW 040426, dont show Comp ID if the traffic is PFLAA, static and a glider
            } else {
              strcpy(info_line,Container[i].Comp_ID);
              display->getTextBounds(info_line, 0, 0, &tbx, &tby, &tbw, &tbh);// kw 081125 - adjustment if close to edge of radar screen
              //Serial.print(Container[i].Comp_ID);
              if (x4+tbw>radar_w){
                x4=x4-tbw;
                y4=y4+5;
              }
              display->setCursor(x4,y4); // all other aircraft
              display->print(info_line);
              //Serial.print("i = ");Serial.print (i); Serial.print("  Container Comp_ID = ");Serial.println(Container[i].Comp_ID);  // KW 080426
           }
          }
          
          
        }else {   // Source=6  mode-S
...
// ===== ON TOW INDICATOR  OTI ========  
      if (GliderOnTow()) {
        FT=1; //set first threat   // KW 180726
        int8_t rv = traffic[0].fop->RelativeVertical;   // integer metres    * _GPS_FEET_PER_METER to convert to feet
        int8_t HighLow = settings->OTI_HighLow; // in metres
        int8_t Danger = settings->OTI_Danger;  // in metres

       

        // -- tow status
        if(rv<HighLow){
          strcpy(info_line, "LOW TOW");
          digitalWrite(SOC_GPIO_PIN_BUZZ_T5S, LOW);  // for warning light or buzzer
        }
        else if(rv>Danger){
          strcpy (info_line,"DANGER");
          digitalWrite(SOC_GPIO_PIN_BUZZ_T5S, HIGH);  // for warning light or buzzer
        }
        else{
          strcpy(info_line, "HIGH TOW");
          digitalWrite(SOC_GPIO_PIN_BUZZ_T5S, LOW);  // for warning light or buzzer
        }
        display->setFont(&FreeMonoBold9pt7b);
        display->getTextBounds(info_line, 0, 0, &tbx, &tby, &tbw, &tbh);  // -- KW 210726
        ontow_y_status = radar_y + radar_w - tbh-3; // tow of text
        display->fillRect(ontow_x-tbw/2 - 3, ontow_y_status-1, tbw +6 ,tbh+4, GxEPD_WHITE);
        display->drawRoundRect( ontow_x -tbw/2 - 4,
                                ontow_y_status -1,
                                tbw+8,
                                tbh+4,
                                4, GxEPD_BLACK);

        display->setCursor(ontow_x - tbw/2, ontow_y_status + tbh);
        display->print (info_line);

        // -- glider on tow ID
        display->setFont(&FreeSerifBold12pt7b);
        if (traffic[0].fop->Comp_ID){
          strcpy (info_line,traffic[0].fop->Comp_ID);
          display->getTextBounds(info_line, 0, 0, &tbx, &tby, &tbw, &tbh);  // -- KW 140926
          ontow_y_ID = ontow_y_status -2 - tbh-2;  // top of text
          display->fillRect(ontow_x - tbw/2 - 1, ontow_y_ID, tbw+2, tbh+4, GxEPD_WHITE);  
          display->drawRoundRect( ontow_x -tbw/2 -3,
                                  ontow_y_ID - 1,
                                  tbw+6,
                                  tbh+5,
                                  4, GxEPD_BLACK);
          
          display->setCursor(ontow_x-tbw/2, ontow_y_ID + tbh);
          display->print(info_line);
          Serial.println("==> OTI end");
        }

#if 1        
        // -- display setting criteria and relative vertical (metres) for testing
        display->fillRect(radar_x+ radar_w *3/4 +3, radar_y+  radar_w *3/4 + 8, radar_w /4 -5, radar_w /4, GxEPD_WHITE);  // KW 200726
        display->setCursor(radar_x + radar_w * 3/4 + 5, ontow_y_ID+12);
        display->setFont(&Picopixel);
        display->print (settings->OTI_Danger);
        display->print (" / ");
        display->print (settings->OTI_HighLow);
        display->setFont(&FreeMonoBold9pt7b);
        //display->print(traffic[0].fop->RelativeBearing);

        display->setCursor(radar_x + radar_w * 3/4 + 5, ontow_y_status +12);
        display->print(rv); 
#endif
      ...
      // -- draw RMI arrow if in View Nav Mode -------------------------
      if (EPD_view_mode == VIEW_MODE_NAV){
        ...
        float epd_RMI[ICON_RMI_POINTS][2];
        float RMI=0;

        switch(settings->orientation){
        case DIRECTION_TRACK_UP:
          RMI = -ThisAircraft.Track + courseTo;
          break;
        default:
          RMI = courseTo;
          break;
        ...
        float rmiSin = sin(D2R * RMI);
        float rmiCos = cos(D2R * RMI);
        
        for (int i=0; i < ICON_RMI_POINTS; i++){
          epd_RMI[i][0] = RMI_Points[i][0];
          epd_RMI[i][1] = RMI_Points[i][1];
              EPD_2D_Rotate(epd_RMI[i][0], epd_RMI[i][1], rmiCos, rmiSin);  // +rmisin, so rotate icon clockwise
        }
        
        display->drawTriangle(radar_center_x + (int16_t) round(epd_RMI[0][0]),
                              radar_center_y + (int16_t) round(epd_RMI[0][1]),
                              radar_center_x + (int16_t) round(epd_RMI[1][0]),
                              radar_center_y + (int16_t) round(epd_RMI[1][1]),
                              radar_center_x + (int16_t) round(epd_RMI[2][0]),
                              radar_center_y + (int16_t) round(epd_RMI[2][1]),
                              GxEPD_BLACK);
        
        display->drawLine(radar_center_x + round(epd_RMI[3][0]),
                            radar_center_y + round(epd_RMI[3][1]),
                            radar_center_x + round(epd_RMI[4][0]),
                            radar_center_y + round(epd_RMI[4][1]),
                            GxEPD_BLACK);
   ...
   //Serial.println("---Timer-check end of lookout "); // WAGA 100425
    ...
    display->setCursor(radar_x+2,radar_y+radar_w-2);
       if (settings->units == UNITS_METRIC || settings->units == UNITS_MIXED) {
          ...
          display->print(EPD_zoom == ZOOM_LOWEST ? "5" :
                        EPD_zoom == ZOOM_LOW    ? "2.5" :
                        EPD_zoom == ZOOM_MEDIUM ? "1" :
                        EPD_zoom == ZOOM_HIGH   ? "0.5" : "");
        } // KW 230726 removed km and nm to unclutter screen
    ...
    case DIRECTION_TRACK_UP:
      ...
      */
      ...
      // draw aircraft heading back bearing bottom centre of radar square
      if (EPD_view_mode == VIEW_MODE_NAV){
        
        backBrg = ThisAircraft.Track - 180;
              if (backBrg < 0)   backBrg += 360;
        if (backBrg > 360) backBrg -= 360;
        //Serial.print ("+++++++++++++++++++++ backBrg = ");
        //Serial.println (backBrg);

        display->setFont(&FreeMonoBold9pt7b);
        snprintf(cog_text, sizeof(cog_text), "%03d", backBrg);
        display->getTextBounds(cog_text, 0, 0, &tbx, &tby, &tbw, &tbh);
        x = radar_x + (radar_w - tbw) / 2;
        y = radar_y + radar_w/2 + radius - (1 * tbh)/2;
        display->setCursor(x , y);
        display->print(cog_text);
        display->drawRoundRect(x-2, y-tbh-2, tbw+8, tbh+6, 4, GxEPD_BLACK);
     }
...
void EPD_radar_loop()
{
    ...
    LoopTime = millis() - LoopLast;
    LoopLast = millis();
    //Serial.print("=> LoopLast = ");Serial.println(LoopLast);

  char info_line [20];
  bool RadarNow = false;
 
   // shortcut, ie EPD update, if there is traffic with an alarm_level>0 or else every 2 (?) seconds
  ...
  Serial.print("EPD_is_ready = ");Serial.println(SoC->EPD_is_ready());  // WAGA debug timings 160425
    ...
    if (RadarNow && SoC->EPD_is_ready()) {
      ...
      if (Threat_Assess()!=0 && traffic[0].fop->alarm_level>0){
          ...
          Serial.println(info_line);  // kw 070426
    ...
    if (hasData) {
      ...
      // ==== blinker to show the device is on
      if (blinker){
        display->fillRect(display->width()-9, display->height()-9,
                          5,5,
                          GxEPD_BLACK);
      }else{
        display->fillRect(display->width()-9, display->height()-9,
      ...
      blinker=!blinker;
      // === display milli seconds since last screen update
      display->setFont(&Picopixel);  //(&FreeMonoBold9pt7b)
      display->setCursor(display->width()-25, display->height()-5);
      display->print(millis()-LastDisplay);
      // === radar loop time
      LastDisplay = millis();
      display->setCursor(display->width()-50, display->height()-5);
...
bool GliderOnTow(){
  if (traffic[0].fop->Comp_ID){
    if (ThisAircraft.GroundSpeed> 20 &&
      EPD_view_mode==VIEW_MODE_RADAR && 
      !(settings->OTI_Danger == 0 && settings->OTI_HighLow == 0) &&
      !(traffic[0].fop->alarm_level > 0) &&
      traffic[0].fop->distance <100){
    return true;
    }
    return false;
  }
