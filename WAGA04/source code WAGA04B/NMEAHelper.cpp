...
char info_line [30];  // WAGA 140425  timing analysis - was 20 prior to 160926
...
//          Serial.print(F(" RelativeDistance=")); Serial.println(NMEA_Status.RelativeDistance);
        ...
        if (S_ID.isUpdated())
        {
//          //Serial.print(F(" ID=")); Serial.print(S_ID.value()); // KW 070426
          ...
          id = fo.ID; 
          int ID_Alarm = fo.alarm_level;
          int dist = fo.distance;
          snprintf(info_line, sizeof(info_line), "LAU ID= %06X A=%d Dist=%d", id, ID_Alarm, dist);  // 190927
