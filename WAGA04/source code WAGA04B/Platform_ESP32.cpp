...
static int ESP32_EPD_ident()
{
  ...
  int rval = EP_GDEW027W3; /* default */ // KW 250826 - change this to set GDEY027T91
#if 0
                            ...
                            char *buf2=NULL, size_t size2=0)
{
  ...
  char key[8];
  char out[64];
  uint8_t tokens[5] = { 0 };   // was [3] - allow room for future inclusion of aircraft type
  cdbResult rt;
  int c, i = 0;
  ...
  if (rt == KEY_FOUND) {
      ...
      case ID_TAIL:
        pref1 = tokens[2];   // CN
        pref2 = tokens[1];   // rego WAGA 030525, was previously pref2 = token[0] M&M
        pref3 = tokens[0];   // M&M WAGA 030525, was previously pref3 = token[1] reg0
        break;
      case ID_MAM:
        pref1 = tokens[0];
        pref2 = tokens[1];
...
static void ESP32_TTS(char *message, int voice)  // WAG 090425 replace 2nd parameter voice
{
    ...
    if ((settings->voice == VOICE_ON || settings->voice == VOICE_TONE) && settings->adapter == ADAPTER_TTGO_T5S){  // kw 130426
      // continue
    } else{
      return;
    }
...
// kw - commented out start
      while (!SoC->EPD_is_ready()) {yield();} //  KW 120426 - this MUST be left else start of voice messages are distorted.
...
// kw commented out end
      ...
      char *word = strtok (message, " "); // split out the first word from the message
      ...
      while (word != NULL)
      {
          ...
          word = strtok (NULL, " ");  // split out the next word from the message
      ...
      if (settings->voice == VOICE_TONE){  // demo TONE output
        strcpy(filename, WAV_FILE_PREFIX);
        strcat(filename, TONE_SUBDIR);
        strcat(filename, "Tone_Alarm3");
        strcat(filename, WAV_FILE_SUFFIX);
        play_file(filename, 0);
      } else {        
        /* demo alarm voice output */
        if (settings->filter !=TRAFFIC_FILTER_ALARM) { 
          // male voice for advisories
          strcpy(filename, WAV_FILE_PREFIX);
        ...
        // demo female voice for warnings
        strcpy(filename, WAV_FILE_PREFIX);
