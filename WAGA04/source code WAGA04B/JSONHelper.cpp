...
// WAGA 150225
JsonVariant portland = root["portland"];
  if (portland.success()) {
    const char * portland_s = portland.as<char*>();
    if (!strcmp(portland_s,"PORTRAIT")) {
      eeprom_block.field.settings.portland = SCREEN_PORTRAIT;
    } else if (!strcmp(portland_s,"LANDSCAPE")) {
      eeprom_block.field.settings.portland = SCREEN_LANDSCAPE;
    } else if (!strcmp(portland_s,"TUG")) {
      eeprom_block.field.settings.portland = SCREEN_TUG;
    }
  }
// 

// WAGA
JsonVariant waypoint = root["waypoint"];
  if (waypoint.success()) {
    const char * waypoint_s = waypoint.as<char*>();
    if (!strcmp(waypoint_s,"YBEV")) {
      eeprom_block.field.settings.waypoint = YBEV;
    } else if (!strcmp(waypoint_s,"YCUN")) {
      eeprom_block.field.settings.waypoint = YCUN;
    } else if (!strcmp(waypoint_s,"YNRG")) {
      eeprom_block.field.settings.waypoint = YNRG;
    }else if (!strcmp(waypoint_s,"YKEP")) {
      eeprom_block.field.settings.waypoint = YKEP;
    } else if (!strcmp(waypoint_s,"YNRM")) {
      eeprom_block.field.settings.waypoint = YNRM;
    } else if (!strcmp(waypoint_s,"YCNK")) {
      eeprom_block.field.settings.waypoint = YCNK;
    }
  }
// 
  ...
  if (voice.success()) {
    ...
    } else if (!strcmp(voice_s,"ON")) {
      eeprom_block.field.settings.voice = VOICE_ON;
    } else if (!strcmp(voice_s,"TONE")) {
      eeprom_block.field.settings.voice = VOICE_TONE;  // KW 130426
    } else if (!strcmp(voice_s,"BUZZER")) {
      eeprom_block.field.settings.voice = VOICE_BUZZER;
