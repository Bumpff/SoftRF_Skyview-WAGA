...
void stop_bluetooth()  // WAGA 250225 - stops wifi when webUI Settings Page is Running
{
  uint32_t freemem = ESP.getFreeHeap();
  Serial.print(F("BT check. Free memory: "));  Serial.println(freemem);
  if (freemem > 60000)
      return;
  if (settings->connection == CON_BLUETOOTH_SPP || settings->connection == CON_BLUETOOTH_LE ) {
      if (SoC->Bluetooth) {
...
void handleSettings() {
  ...
  size_t size = 6000; // KW 070826 was 5000
  char *offset;
  size_t len = 0;
  ...
  Serial.println(F("handleSettings()...")); // WAGA 250425
  stop_bluetooth(); // stop bluetooth if short of memory WAGA 250425
...
<option %s value='%d'>YKEP</option>\
<option %s value='%d'>YNRM</option>\
<option %s value='%d'>YCNK</option>\
</select>\
</td>\
</tr>\
<tr>\
...
<tr>\
<th align=left>OTI Danger (m)</th>\
<td align=right>\
<INPUT type='number' name='OTI_Danger' min='0' max='100' value='%d'>\
</td>\
</tr>\
<tr>\
<th align=left>OTI HighLow (-m)</th>\
<td align=right>\
<INPUT type='number' name='OTI_HighLow' min='-100' max='0' value='%d'>\
</td>\
</tr>"),
  ...
  (settings->waypoint == YKEP  ? "selected" : ""), YKEP,
  (settings->waypoint == YNRM  ? "selected" : ""), YNRM,
  (settings->waypoint == YCNK  ? "selected" : ""), YCNK,
...
//<option %s value='%d'>voice 3</option>
    ...
    (settings->voice == VOICE_TONE? "selected" : ""), VOICE_TONE, // KW 130426
...
<p align=center><INPUT type='submit' value='Reset'></p>\
</form>\
</body>\
</html>"),
...
void handleInput() {
  ...
  for ( uint8_t i = 0; i < server.args(); i++ ) {
    ...
    } else if (server.argName(i).equals("OTI_Danger")) {
      settings->OTI_Danger = server.arg(i).toInt();

    } else if (server.argName(i).equals("OTI_HighLow")) {
      settings->OTI_HighLow = server.arg(i).toInt();
...
<tr><th align=left>OTI Danger</th><td align=right>%d</td></tr>\
<tr><th align=left>OTI HighLow</th><td align=right>%d</td></tr>\
...
<hr>\
  ...
  settings->waypoint, settings->OTI_Danger, settings->OTI_HighLow, settings->adb, settings->idpref, settings->voice, settings->aghost,
...
void Web_setup()
{
  ...
  server.on("/firmware", HTTP_GET, [](){
    ...
    Serial.println("-- about to run stop_bluetooth.  Web setup--");
    //stop_bluetooth();  // WAGA 060525
