...
==============
1. For towplanes: Added 'On-Tow Indicator' (OTI).  It shows Comp ID of glider on tow and vertical tow position.  Derived from FLARM RelativeVertical data, shows 'Low' (below slip stream), 'High' (above slip stream but not dangerous) or 'Danger' (tow position too high, may cause tug upset).  Proven to works with tow ropes of 75m or more. New 'OTI Danger' and 'OTI HighLow' values (metres) settings in Web GUI.  If both set to zero, OTI is deactivated (correct settings for gliders).  See document explaining the theory and limitations.  Danger warning may be delayed upto 2 seconds after event due to ePaper refresh time. CAUTION:  The OTI has not been tested with SoftRF devices as the source of RelativeVertical data.
...
Sim tool update 17 May 26.
-------------------------
Improved detection of which Port FLARM is connected to and the FLARM's baudrate, with auto setting to match.
CARP analysis can now cope with nil data after reset  (lack of start/end date was causing error).
