...
int EPD_view_mode = 0; // WAGA 070325 was static before declare external in .h
int EPD_portland = 0;  // WAGA added 150225
...
byte EPD_setup(bool splash_screen)
{
  ...
  int16_t  tbx5, tby5;
  uint16_t tbw5, tbh5;
  uint16_t rect_x, rect_y;  // WAGA 170225

  EPD_view_mode = settings->vmode; 
  EPD_portland = settings->portland;
  ...
  if (display) {
    ...
    if (splash_screen) {
...

      {
        ...
        uint16_t y = (display->height() + tbh1) / 3 - 4; // amend WAGA 090225, was /2
        display->setCursor(x - (tbw1 / 3), y - tbh1);
        display->print(EPD_SkyView_text1);
        x = (display->width() - tbw2) / 2;
        y = (display->height() + tbh2) / 3 - 4; // amend WAGA 090225, was /2
        ...
        y = (display->height() + tbh3) * 2 / 4 - 4; // amend WAGA 090225, was 3/4-4
  ...
  if (hw_info.display == DISPLAY_EPD_2_7) {
    ...
    if (!EPD_display_frontpage) {
      ...
      case VIEW_MODE_TEXT:
        ...
        break;
      default:
        EPD_radar_loop(); // WAGA 070325 applies in case VIEW_MODE_NAV
        break;
      }
    }
  }
...
void EPD_Mode()
{
  if (hw_info.display == DISPLAY_EPD_2_7) {
    ...
    if (EPD_view_mode == VIEW_MODE_RADAR) {
      ...
      EPD_view_mode = VIEW_MODE_NAV;
      EPD_radar_setup();
      EPD_display_frontpage = false;
    } else if (EPD_view_mode == VIEW_MODE_NAV) {
