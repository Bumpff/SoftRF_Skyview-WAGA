...
// here print Navbox1.title title
    ...
    switch(EPD_view_mode){
      ...
      if (Threat_Assess()>0) {
          ...
          if (traffic[0].fop->distance * disp_dist <1000 && settings->units !=UNITS_IMPERIAL){
            navbox2.value = traffic[0].fop->distance *disp_dist;
  ...
  //====== BOTTOM NAV BOXES ===========
  {
    ...
    switch(EPD_view_mode){ 
      case VIEW_MODE_RADAR:
      {
        ...
        display->print(traffic[0].fop->Comp_ID);
        /*
...
//=====================================================================
      ...
      if(Container[i].Source!=6)
      {
        ...
        // draw target as a simple triangle
          ...
          display->getTextBounds(tail, 0, 0, &tbx, &tby, &tbw, &tbh);// kw 081125 - adjustment if close to edge of radar screen
          if (x4+tbw>display->width()){
            x4=x4-tbw;
            y4=y4+5;
          }
